...
#
# Writes: dist/vib-ribbon-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix vib-ribbon \
  --exe-name Vib_Ribbon_Recompiled \
  --display-name "Vib-Ribbon Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Vib-Ribbon (Europe) (En,Fr,De,Es,It).cue" \
